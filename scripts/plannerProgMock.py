...
class Loss():
    ...
    def __init__(self, vpsto, bounds, dt, T_max, predSigs, startTime, boundLossWeight, cppCost, shortHorz, pos_key='pos', vel_key='vel'):
        self.bounds = bounds
    ...
    def loss_stl_cpp(self, pop):
        ...
        costs = stlrom.batch_find_robustness(
            self.driver, futureSignalTiled, "phi", self.robsInd)
        costs = np.array(costs)
        return np.array(costs)
...
class planner:
    ...
    def __init__(self):
        ...
        # create loss class
        self.loss = Loss(self.vpsto, bounds, self.dt, self.Tmax,
        ...
        self.remainingFormula = ""

        # get x trigger
        self.myXtrig = rospy.get_param("/xTrigValue")

        self.awaitingTrigger = True
        ###############################
        # initialize node information #
        ###############################
        # define topics to publish to
        self.goalTopic = rospy.Publisher(
            'progMockTraj', Float64MultiArray, queue_size=10)
        self.logExecTime = rospy.Publisher(
            'progMockExecTime', Float64, queue_size=10)
        self.logCost = rospy.Publisher(
            'progMockCost', Float64, queue_size=10)

        # initialize node
        rospy.init_node('planner', anonymous=True)

        # define subscriber callbacks
        rospy.Subscriber('remainingFormulaMock', String,
                         self.process_remaining_formula)
        rospy.Subscriber('stateObservationsMock', Float64MultiArray,
                         self.observe)
        rospy.Subscriber('planningMock', Bool, self.check_planning)

    def process_remaining_formula(self, data):
        """save the most recent STL formula-to-go"""
        self.remainingFormula = data.data

    def observe(self, data):
        """receive and store the true state of the system"""
        self.recentState = np.reshape(np.array(data.data), [1, 7])
        if (self.recentState[0, 1] > self.myXtrig) & self.awaitingTrigger:
            self.planning = True
            self.awaitingTrigger = False
    ...
    def run(self):
        ...
        while not rospy.is_shutdown() and self.programRunning:
            if self.planning:
                ...
                snapshot = np.copy(self.recentState)
                ...
                if size > 0:
                    self.loss.predSigs = snapshot[0, 5:]
                    # update everything for next loop
                    self.startPos = snapshot[0, 1:3]
                    self.startVel = snapshot[0, 3:5]
                    self.realTime = snapshot[0, 0]
                    if not (self.shortHorz):
                        self.loss._N_eval_max = round(
                            (self.Tmax-self.realTime) / self.dt) + 1
                        self.loss.t = np.linspace(
                            0, self.Tmax-self.realTime, self.loss._N_eval_max)
