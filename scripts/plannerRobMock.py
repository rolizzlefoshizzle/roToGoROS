...
class planner:
    ...
    def __init__(self):
        ...
        self.goalTopic = rospy.Publisher(
            'robMockTraj', Float64MultiArray, queue_size=10)
        self.logExecTime = rospy.Publisher(
            'robMockExecTime', Float64, queue_size=10)
        self.logCost = rospy.Publisher(
            'robMockCost', Float64, queue_size=10)
    ...
    def observe(self, data):
        ...
        newState = np.reshape(np.array(data.data), [1, 7])
        self.trueHistory = np.vstack(
            (self.trueHistory, newState))
        if (newState[0, 1] > self.myXtrig) & self.awaitingTrigger:
            self.planning = True
